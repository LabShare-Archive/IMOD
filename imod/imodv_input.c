...
void imodv_input_cb(Widget w, XtPointer client, XtPointer call)
{
	...
	case KeyPress:
	  ...
	  switch(keysym){
	     ...
	     case XK_comma:
	       newval = a->md->arot / ROTATION_FACTOR + 0.5;
	       if (newval == a->md->arot)
		    newval--;
	       if (!newval)
		    newval = 1;
...
void imodv_rotate(ImodvApp *a, int throw)
{
     int mx, my, idx, idy;
     unsigned int maskr = imodv_query_pointer(a,&mx,&my);
     float dx, dy;
     ...
     if (a->movie && !(maskr & ControlMask)) {
		  ...
		  movement since the button was pressed */
	       ...
	       idx = MOUSE_TO_THROW * dy + 0.5;
	       idy = MOUSE_TO_THROW * dx + 0.5;
	       if (!idx && !idy)
		    a->movie = False;
	       a->md->xrotm = idx;
	       a->md->yrotm = idy;
     ...
     dx = (mx - a->lmx);
     dy = (my - a->lmy);
     idx = MOUSE_TO_ANGLE * dy + 0.5;
     idy = MOUSE_TO_ANGLE * dx + 0.5;
     if ((!idx) && (!idy))
	  return;
