		...
		for(i  = 0; i < mesh->lsize; i++){
			...
			while(mesh->list[++i] != IMOD_MESH_ENDPOLY){
			    ...
			    fprintf(fout, "%g %g %g %g %g %g\n",
				    mesh->vert[i+1].x * mod->xscale,
				    mesh->vert[i+1].y * mod->yscale,
				    mesh->vert[i+1].z * mod->zscale,
				    mesh->vert[i].x * mod->xscale,
				    mesh->vert[i].y * mod->yscale,
				    mesh->vert[i].z * mod->zscale);
			    i+=2;
			    fprintf(fout, "%g %g %g %g %g %g\n",
				    mesh->vert[i+1].x * mod->xscale,
				    mesh->vert[i+1].y * mod->yscale,
