...
[[ifnot = build]]
	...
	if.not.var.montage.assert.disabled.cb.reduce-size-with-antialiasing-filter=on
	bn.create-full-aligned-stack=
	if.not.var.montage.wait.process.creating-aligned-stack=done
	if.var.montage.wait.process.full-alignment=done
	# check files
  ...
  assert.contains.file=newst%{axis}.com|AntialiasFilter*-1
[[]]
# run unbinned
if.not.var.montage.assert.disabled.cb.reduce-size-with-antialiasing-filter=off
sp.aligned-image-stack-binning=1
bn.create-full-aligned-stack=
if.not.var.montage.wait.process.creating-aligned-stack=done
if.var.montage.wait.process.full-alignment=done
