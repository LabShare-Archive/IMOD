...
assert.cb.align-averages-to-have-their-y-axes-verticle=off
assert.cb.use-absolute-value-of-cross-correlation=off
assert.cb.save-individual-aligned-particles=off
assert.sp.particles-per-cpu=5
assert.sp.debug-level=3
assert.tf.low-frquency-cutoff=0
assert.tf.sigma=
...
save=
assert.contains.file=%{dataset}.prm|flgAbsValue = 1
...
sp.particles-per-cpu=5
sp.debug-level=2
save=
assert.contains.file=%{dataset}.prm|debugLevel = 2
sp.debug-level=3
tf.low-frequency-cutoff=1
tf.sigma=2
save=
assert.contains.file=%{dataset}.prm|lowCutoff = [1,0]
tf.low-frequency-cutoff=0
tf.sigma=
save=
