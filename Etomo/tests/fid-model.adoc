...
[[if = test-gui]]
  #basic
  #test values
  assert.cb.make-boundary-model=off
  ...
  #test enabled/disabled
  assert.enabled.cb.make-boundary-model=
  assert.disabled.bn.open-boundary-model=
  ...
  #behavior
  cb.make-boundary-model=on
  assert.enabled.bn.open-boundary-model=
  assert.enabled.cb.exclude-inside-boundary-contours=
  cb.make-boundary-model=off
  #
  rb.density=
  ...
  assert.tf.fraction-of-peak-to-store=1.0
  ...
  assert.enabled.tf.fraction-of-peak-to-store=
...
[[if = test-gui]]
  #test values
  bn.generate-seed-model=on
  #test enabled/disabled
  bn.enabled.open-initial-bead-model=
  bn.disabled.open-sorted-3d-models=
  #files
  assert.exists.file=autofidseed%{axis}.com
  assert.exists.file=autofidseed%{axis}.info
  assert.exists.file=%{dataset}%{axis}.seed
  assert.exists.file=autofidseeda%{axis}.dir
  assert.exists.file=autofidseeda%{axis}.log
  assert.exists.file=autofidseeda%{axis}.dir/afs*.pkmod
  assert.exists.file=autofidseeda%{axis}.dir/afs*.*.elong
  assert.exists.file=autofidseeda%{axis}.dir/afs*.*.track
  assert.exists.file=autofidseeda%{axis}.dir/track%{axis}.com
[[]]
...
[[if = test-gui]]
  #test enabled/disabled
  bn.enabled.open-sorted-3d-models=
  #files
  assert.exists.file=%{dataset}%{axis}.seed~
  assert.exists.file=autofidseeda%{axis}.log~
  assert.exists.file=autofidseeda%{axis}.dir/afs*.*.xyzmod
  assert.exists.file=autofidseeda%{axis}.dir/afs*.*.sortmod
[[]]
bn.generate-seed-model=
wait.process.autofidseed=done
[[if = test-gui]]
...
[[if = test-gui]]
  #test enabled/disabled
  bn.disabled.open-initial-bead-model=
  bn.disabled.open-sorted-3d-models=
  #files
  assert.not-exists.file=autofidseeda%{axis}.dir
[[]]
