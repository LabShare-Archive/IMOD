 ...
 * @version $Revision$
 */
public final class UITest extends JFCTestCase implements UITestCommandFactory {
  ...
  public void test() throws IOException, LogFile.ReadException,
    ...
    reader = new CommandReader(autodoc, TestSectionCommand.SECTION_TYPE);
    CommandFactory factory = new CommandFactory();
    UITestAutodocCommand command = (UITestAutodocCommand) reader.nextCommand(
        null, factory);
    ...
    while (!command.isEmpty()) {
      ...
      command = (UITestAutodocCommand) reader.nextCommand(command, factory);
    }
    ...
    processSection(autodoc);
    UIHarness.INSTANCE.setLog(true);
    runTests();
  }

  private void setSleep(UITestAutodocCommand command) {
  ...
  public UITestCommand newCommand() {
    return new TestSectionCommand();
  }

  private void setDuration(TestSectionCommand command) {
  ...
  private String getRequiredValue(TestSectionCommand command) {
    String value = command.getValue();
    assertNotNull(reader.getInfo(), "Unknown name/value pair format: "
        + command, value);
    return value;
  ...
  private void setVariable(TestSectionCommand command) {
  ...
  private ArrayList setVariable(TestSectionCommand command, ArrayList variables) {
    if (variables == null) {
  ...
  private void setDataset(TestSectionCommand command) {
    String value = command.getValue();
    assertNotNull(reader.getInfo(), "Unknown name/value pair format: "
        + command, value);
    dataset = value;
   ...
   * @throws IOException
   */
  private void processSection(ReadOnlyAutodoc autodoc)
      throws FileNotFoundException, IOException, LogFile.ReadException,
      LogFile.FileException {
    TestSectionCommand command = (TestSectionCommand) reader.nextCommand(null,
        this);
    while (!command.isEmpty()) {
      if (command.isKnown()) {
        UITestAction action = command.getAction();
        ...
        else if (action == UITestAction.INTERFACE) {
          interfaceSection = InterfaceSection.getInstance(autodoc, command);
        }
      }
      command = (TestSectionCommand) reader.nextCommand(command, this);
    }
  ...
  private void runTests() {
    ...
    if (autodocA != null) {
      axisAUITest = new UITestAxis(this, autodocA, helper, axisIDA,
          dataFileName != null, variablesA, interfaceSection);
    }
    if (autodocB != null) {
      axisBUITest = new UITestAxis(this, autodocB, helper, axisIDB,
          dataFileName != null, variablesB, interfaceSection);
    }
   ...
   * @return
   */
  boolean removeDialog(AxisID axisID, String dialogSectionName) {
    ...
    return axisUITest.removeFinishedDialog(dialogSectionName);
  }
   ...
   * @param sourceDir
   */
  void copyFile(UITestCommand command) {
   ...
   * @throws IOException
   */
  private void setAutodoc(TestSectionCommand command)
      throws FileNotFoundException, IOException, LogFile.ReadException,
      LogFile.FileException {
    String value = command.getValue();
  ...
  private void setDatasetDir(TestSectionCommand command) {
  ...
  private File getAbsoluteDir(UITestCommand command, boolean makeDirs) {
    //Get the directory name
    String dirName = command.getValue();
    assertNotNull(reader.getInfo(), "Unknown name/value pair format: "
        + command, dirName);
   ...
   * @return
   */
  private File getRelativeDir(UITestCommand command, File rootDir,
      boolean makeDirs) {
  ...
  private static final class CommandFactory implements UITestCommandFactory {
    public UITestCommand newCommand() {
      return new UITestAutodocCommand();
    }
  }
}
/**
