...
public class FilePathTest extends TestCase {
  ...
  public void testGetPath() {
    ...
    assertTrue("Should return the to-path when the drives are different", FilePath
        .getRelativePath(xyzPath, xyzPathWithADrive).equals(xyzPathWithADrive));
    assertTrue("Should return the to-path when the drives are different", FilePath
        .getRelativePath(xyzPathWithADrive, xyzPath).equals(xyzPath));
    assertTrue("Should return the to-path when the drives are different", FilePath
        .getRelativePath(xyzPathWithADrive, xyzPathWithBDrive).equals(xyzPathWithBDrive));
    assertTrue("Should return the to-path when the from-path is empty",
        FilePath.getRelativePath(null, xyzPath).equals(xyzPath));
    assertTrue("Should return the to-path when the from-path is empty",
        FilePath.getRelativePath("", xyzPath).equals(xyzPath));
    assertTrue("Should return the to-path when the from-path is empty",
        FilePath.getRelativePath(" ", xyzPath).equals(xyzPath));
    assertTrue(
        "Should remove the from-path when it is a subset of the to-path, and return a relative path",
        FilePath.getRelativePath(xyPath, xyzPath).equals(relZPath));
    FilePath.setDebug(true);
    assertTrue("Should go up the from-path where it is not equal",
        FilePath.getRelativePath(xyzPath, abcPath).equals(relUp3 + abcPath));
    ...
    assertTrue(
        ...
        FilePath.getRelativePath(xyzPath, xyPath + abcPath).equals(relUp1 + abcPath));
    assertTrue(
        ...
        FilePath.getRelativePath(xyzPath, abcPath + xyzPath).equals(relUp3 + abcPath + xyzPath));
    assertTrue("Should go up the from-path where it is not equal",
        FilePath.getRelativePath(xPath, aPath).equals(relUp1 + aPath));
  }
}
