...
void main(int argc, char **argv)
{
	...
	switch (ch){
	  ...
	  case 'd':
	    meshDiameterSize = atof(optarg);
	    break;
	  case 'o': /* select object list */
	    list = parselist(optarg, &nlist);
	    if (!list) {
...
/*	    reduce = atoi(optarg); */
	  ...
	  case 'D': /* Do not cap at Z in list. */
	    cap_skip_zlist = parselist(optarg, &cap_skip_nz);
	    if (!cap_skip_zlist) {
	  ...
	  case 't':
	    tube_list = parselist(optarg, &ntube_list);
	    if (!tube_list) {
	  ...
	  case 'z':
	    sscanf(optarg, "%d%*c%d%*c%d", &minz, &maxz, &incz);
	    break;
	  case 'i':
	    incz = atoi(optarg);
	    if (incz < 1)
...
/* Save backup of Model to Model~ */
#ifndef __vms
	rst = (char *)malloc(strlen(argv[i]) + 2);
	if (!rst){
	    fprintf(stderr, "%s: Error, couldn't create backup", argv[0]);
	    exit(-1);
	}
	rst[strlen(argv[i])] = '~';
	rst[strlen(argv[i])+1] = 0x00;
	for(ch = 0; ch < strlen(argv[i]); ch++)
	    rst[ch] = argv[i][ch];
	rename(argv[i], rst);
	free(rst);
#endif
	...
	fout = fopen(argv[i], "w");
	if (!fout){
	    fprintf(stderr, "%s: Error, couldn't open output.", argv[0]);
	    exit(-1);
	}
