cccccccccc subroutine read_mod ccccccccccccccccccccccccccccccccccccccccccccccccc
c  This routine reads model info from a disk file which is generated by
c  STORE_MOD.
c			Hans Chen	Aug 31, 1987
c   DNM 9/12/88 defined new parameters, fixed it so it could read in old models
c   with branch points marked by 10000 but eliminate the marker
...
100	read(20,101,err=151)string
	if(string(indstr:3+indstr).eq.' Obj')then
	  read(string(indstr:),'(10x,i)',err=151)i
	  read(20,101,err=151)string
	  read(string(indstr:),'(11x,i)',err=151)ninobj
	  read(20,101,err=151)string
