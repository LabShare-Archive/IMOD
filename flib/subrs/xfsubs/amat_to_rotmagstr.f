...
c
      ...
      a22=amat(2,2)
c       
c       first determine if there is an axis inversion: find angle from
c       transformed X axis to transformed Y axis and reduce to -180 to 180
c       If difference is negative then invert Y components of matrix
c       
      dtheta = atan2d(a22, a12) - atan2d(a21, a11)
      if (dtheta .gt. 180) dtheta = dtheta - 360
      if (dtheta .le. -180) dtheta = dtheta + 360
      if (dtheta .lt. 0) then
        a12 = -a12
        a22 = -a22
      endif
c	
c       next find the rotation angle theta that gives the same solution for
...
c       
c       Now if there is an inversion, then invert the stretch, mirror the
c       stretch axis, and add a rotation to bring inverted point along stretch
c       axis to a point mirrored around X
c       
      if (dtheta .lt. 0) then
        str = -str
        phi = -phi
        theta = theta + 180. - 2. * phi
        if (theta .gt. 180.) theta = theta - 180.
      endif
c       
c       
