...
c
	subroutine segment_to_triangle(xs,ys,zs,xn,yn,zn,itri,t,xrot,
     &	    yrot,dist)
	include 'mtk.inc'
	logical inside_triangle,sin,nin
c	  
...
c	    
	  if(zsr.gt.zrot(itri).xor.znr.gt.zrot(itri))then
	    dist=0.
...
c
	subroutine triangle_to_triangle(itri1,itri2,xr1,yr1,zr1,xr2,yr2,
     &	    itrir,dist)
	include 'mtk.inc'
	logical inside_triangle,vint(3,2)
...
c
     ...
     &		(zr(iv,it).gt.zrot(itri).xor.zr(ivn,it).gt.zrot(itri)))then
	      dist=0.
