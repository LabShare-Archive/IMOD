...
D	WRITE(*,*)'RETURN FROM EVLPOL, MORE=',MORE
	ENDDO
	GOTO 1
	END
...
C
	BYTE STRING(132),STRING2(132),MODES(10,10),DEFSTR(132,10)
...
c	EQUIVALENCE (STRING,STRNG),(MODES,MODS)
	...
	LOGICAL*1 INDFLG,DEGFLG,DFLAG
	byte VARNAMS(10,50)
	DATA INDFLG,DFLAG/.FALSE.,.FALSE./
	...
	IFFLG=1
	IF (NIN.EQ.0) GOTO 2
C1	IF (.NOT.INDFLG.AND.IFFLG.EQ.0) WRITE (NOUT,'(''$Data value:'')')
1	NDONE=0
	IF (NIN.EQ.0) RETURN
	IF (.NOT.INDFLG) WRITE (NOUT,'(1x,''Calc? '',$)')
2	IF (INDFLG) GOTO 8
	IF (NIN.EQ.0) THEN
c		ISTAT=LIB$GET_FOREIGN(STRNG,,LENGTH,IFLAG)
...
7	    INDFLG=.TRUE.
	    JFLAG=1
8	    READ(94,'(A)',ERR=9,END=10) STRNG
	    length = lnblnk(strng)
	    IF (LENGTH.EQ.0) GOTO 8		
	    IF (STRNG(1:1).EQ.'*'.OR.STRNG(1:1).EQ.'!') GOTO 8!Ignore these
...
c19	      WRITE(*,*)' Meaningless garbage ignored'
c	      GOTO 1
c	    ENDIF
...
D	WRITE (6,455) (STRING2(III),III=1,I-1)
D455	FORMAT(' CALLING EVAL WITH STRING:',100A1)
	CALL EVAL(STRING2,JERR)
	IF (JERR.NE.0) GOTO 1
	LINO=1
22	CALL EVLPOL(RESULT,JERR,MORE)
C	IF (IPOS.EQ.0) RETURN			!Not used in deskcalc
D	WRITE(*,*)'RETURNED FROM EVLPOL, RESULT,JERR,MORE=',RESULT,JERR,MORE
	NDONE=1
	JERR=0
...
C	  
	DO I=1,NFUNCS				!See if function precedes
	  IP=ILP-1
	  IFL=1
...
51	IF (ILP.GT.1.OR.IRP.LT.LENGTH) GOTO 5	!Loop until outer parens found
	...
	ENDDO
100	CONTINUE
D	WRITE (6,143) ISP,(I,STACK(I),I=1,ISP)
D143	FORMAT(' STACK LENGTH=',I5,' CONTENTS-'/(I5,E20.5))
...
D	    IF (IT.NE.0) WRITE(*,*)'IT NE 0, GOING TO 99'
...
C	PUSH AN OPERAND ON THE STACK
	INTEGER*2 STPOS(132)
	BYTE STRING(134),VARNAMS(10,50),TERM(20),DEGFLG
	DATA STBRK,OPCODE,VARCODE,DUNDEF/-3.9E21,-3.9E22,-3.9E23,-3.9E24/
...
10	JFIRST=1
	...
	DO I=1,NVARS				!Else search for variable
	  DO J=JFIRST,IT
...
C	DEFINE A VARIABLE
	INTEGER*2 STPOS(132)
	BYTE VARNAMS(10,50),TERM(20),string(134)
	DATA STBRK,OPCODE,VARCODE,DUNDEF/-3.9E21,-3.9E22,-3.9E23,-3.9E24/
	logical*1 degflg
	COMMON /EVAL2/ ISP,IOP,IERR,STPOS,IP,IRP,ILP,STACK(1000),STRING
	COMMON /VARS/ NVARS,VARNAMS,VL1(50),VL2(50),VL3(50),VL4(50),DEGFLG
...
10	DO IPOS=1,NVAR1				!Now search for the name
		DO J=1,IT
