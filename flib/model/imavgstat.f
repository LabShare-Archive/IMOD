...
c       
c       Before running the program, one uses IMOD to construct a model in
c       which each contour specifies a "summing region".  A summing region
c       may be defined by either 2 points or 4 or more points.  Four points
c       define a quadrilateral summing region (a 5th point to make the model
c       contour look like a quadrilateral is optional and is ignored by the
c       program). Two points define a line; the actual extent of the summing
c       region perpendicular to this line is specified when one runs the
c       program. This model should be built on an image stack in which all
c       of the images being compared are aligned.  More than 4 points can be
c       used to specify a region of complex shape.
c       
c       The summing regions are used in the program in two different ways: to
c       specify the low and high density normalizing areas, and to specify
c       the summing areas, areas that are being compared between images.
c       
c       The normalizing areas are used to scale the averages from all of the
c       different sample sets so that the mean density in the low area is 0
c       and the mean density in the high area is 100.  Each of these two
c       areas may be a combination of more than one summing region.  However,
c       all of the regions used to specify a low or high normalizing area
c       must be described by contours of at least 4 points.  If you do not
c       want the densities normalized, enter one contour for the low
c       normalizing area and the same contour for the high normalizing area.
c       In this case, average densities will have the same scaling as the
c       original data.
c       
c       Summing areas may be whole summing regions or subdivisions of summing
c       regions.  When running the program, one specifies the number of
c       summing areas that each region should be divided into.  If a region
c       is a quadrilateral, it will be divided into areas by lines parallel
c       to the short axis of the quadrilateral.  A region specified by 2
c       points will be divided into areas at points equally spaced along the
c       line connecting the two points.  The shape of those areas is
c       specified by a single parameter: 0 for circular areas; 1 for square
c       areas with edges parallel and perpendicular to the connecting line;
c       or, for rectangular areas, the ratio of width to height (height being
c       the dimension parallel to the connecting line).  If you want to
c       divide up a region with more than 5 points, then you must trace the
c       region in a special way.  Start at one end of the region and trace
c       one side along the long axis, with as many points as needed to define
c       the region adequately.  At the other end, add a single line segment
c       to describe that end, then trace back along the other long side,
c       with the same number of points as on the first long side, and with
c       each point opposite the corresponding point on the other side.  Both
c       short ends must thus be decsribed by a single line segment.
c       
c       You can divide a region into areas by specifying either the number
c       of areas to divide it into, or the desired width (in pixels) of each
c       of the subdivisions.  In the latter case, you enter the negative of
c       the width in pixels.
c       
c       The program has an option to produce an image file with different
c       colored pixels for each summing area; one can use this while
c       learning how to produce summing regions at desired locations.
c       
c       The program can compute statistics and averages from unaligned
c       samples within each data set, applying any needed transformations in
c       a single step rather than in the 2 or 3 steps that would ordinarily
c       be used to get a series of aligned averages.  The averages produced
c       by this program might thus be superior in quality or appearance.  For
c       each data set, one can specify 1 or 2 sets of transforms that are
c       needed to align all of the samples within the data set.  If multiple
c       data sets are being used (or if the summing region model is built on
c       an average that was transformed to align with other averages), then
c       one must specify in addition the G transform applied to the average
c       to align each data set with other averages or with the model.
c       
c       The program requests numerous entries of "lists", in which "ranges
c       are OK".  In such a list, a range is specified by 2 numbers separated
c       by a dash, and ranges or individual values are separated by commas.
c       For example, 2-4,7,9,11-14 specifies the list 2,3,4,7,9,11,12,13,14
c       When you put entries into a command file, you may put text after a
c       list. 
c       
c       The program allows you to append its output to a set of existing
c       files from a previous run of the program.  If you elect to append,
c       then the files (statistics output, average image, and standard
c       deviation image) will first be copied to new versions, then output
c       will be appended to those new files.  If the program crashes, the
c       previous versions will be intact, but you must be careful to delete
c       any new, incorrect versions of the files before trying to run the
c       program again.  Be sure NOT TO PURGE before doing this.
c       
c       Entries to the program:
c       
c       0 to place output in new files, or 1 to append to existing files.
c       
c       Name of model file defining summing regions, or Return if no
c       .  summing or normalizing is desired
c       
c       Name of file of G transforms used to align averages from different
c       .  data sets to each other.  Enter Return if no G transforms.
c       
c       Name of file to output the statistics into, or Return for no output.
c       .  This file is readable, but is meant to be run into AVGSTATPLOT
c       
c       Name of file to place new average images into, or Return for none
c       
c       Name of file to place standard deviation images into, or Return
c       .  for none
c       
c       IF you did not enter a model file with summing areas, skip the next
c       .  6 entries and go right to entering NX and NY:
c       
c       Name of file for a map of the pixels in the summing areas, or Return
c       .  for none.
c       
c       List of contours that comprise the LOW density normalizing area.
c       .  Here and in the next two entries, enter a series of pairs of
c       .  numbers, all on one line.  The pair is either an IMOD object
c       .  number and contour number, or a WIMP object number and 0.
c       
c       List of contours that comprise the HIGH density normalizing area.
c       .  To avoid normalization, enter the same contour as low and high
c       .  normalizing area.
c       
c       List of contours specifying the summing regions.
c       
c       Number of summing areas in each of the regions just specified, or the
c       .  negative of the width (in pixels) of the areas that you want the
c       .  region divided into.  (You must enter one value per region.)
c       
c       0 for circles, 1 for squares, or ratio of width to length for
c       .  rectangles.  You must enter one value per region; just enter 0 for
c       .  regions specified by 4 or more points.
c       
c       Horizontal and vertical pixel dimensions of the image file (NX and
c       .  NY).
c       
c       Number of data sets to analyse and compare
c       
c       Number of subsets of positions to average for each data set.  Enter /
c       .  if you do not want to do subsets of positions; otherwise enter a
c       .  number for each data set (use zero for no subsets for a particular
c       .  set.)
c       
c       --- The rest of the entries are required for each data set in turn:
c       
c       Name of image file containing stack of samples to be averaged
c       
c       List of section numbers to "try" to include in the averaging,
c       .  or / for all sections.  Ranges may be entered.
c       
c       Number of sets of F transforms to apply to the samples before
c       .  averaging.  Enter 0, 1 or 2; do not count the G transforms
c       .  specified above.
c       
c       IF you specified 1 or 2 sets of F's, next enter the name of the only
c       .  or first file of F transforms
c       
c       IF you specified 2 sets of F's, next enter the name of the second
c       .  file of F transforms
c       
c       IF you specified any F's, next enter the offset to add to the section
c       .  number to obtain the line number of the corresponding transform in
c       .  the file of F's.  Both line and section numbers start at 0.
c       .  If alignment routines have been used properly, an entry of 0 will
c       .  suffice.
c       
c       IF you specified a file of G transforms to align different sets to
c       .  each other, next enter the line number of the G transform for this
c       .  data set.  The first line is number 0.
c       
c       IF you specified that you wanted to average subsets of positions for
c       this data set, next make the following entries:
c       
c       .   Name of file with list of position numbers for each section, as
c       .      produced by EXTPOSITION
c       
c       .   For each subset, enter a list of position numbers to include in
c       .      the average.  Enter each list on a separate line.
c       
c       Enter 1 to set cutoffs for elimination of outliers, -1 for automatic
c       .  selection of cutoffs, or 0 to skip this option.  If you do select
c       .  this option, the program enters a loop (with entries described
c       .  below) in which it repeatedly comes back to this point until you
c       .  enter a 0.
c       
c       --- An entry of 0 at the last step completes the entries for a data
c       --- set; you then enter all parameters for the next data set, etc.
c       
c       The last option allows you to interactively eliminate "outliers",
c       sections that deviate the most from the average in the low and/or
c       high normalizing areas or in the difference between high and low
c       areas.  This option should be used only if one has a specific basis
c       for thinking that some subset of sections are significantly poorer
c       than the rest.  Otherwise, it is strongly recommended that you skip
c       through this option by entering 0.
c       
c       Outliers can be eliminated based one whether their low normalizing
c       area is more than a criterion number of standard deviations away from
c       the mean for all samples, or on whether the high normalizing area
c       deviates from the mean by more than a separate criterion, or on
c       whether the difference between high and low areas deviates by more
c       than yet another criterion.  If one enters a criterion of zero for
c       one of these 3 deviations, that deviation will not be considered.
c       One may elect to eliminate outliers only if all deviations being
c       considered are above their respective criteria, or if any of those
c       deviations are above criterion.
c       
c       If you do select manual elimination of outliers (with an entry of 1),
c       then there two entries:
c       
c       Criterion number of S.D.'s for deviation from mean of low area, of
c       .  high area, and of difference between low and high areas.  The
c       .  default is 2,2,2.
c       
c       0 to eliminate a section if any of deviations being considered are
c       .  over criterion, or 1 to eliminate only if all deviations are over.
c       
c       If you select automatic elimination of outliers with an entry of -1,
c       then there are no further entries.  The program will then attempt to
c       find the outlier elimination that minimizes the sum of the standard
c       errors of the mean of all of SUMMING (not normalizing) areas.  It
c       does this by repeatedly scaling the last-entered values of the three
c       criteria (or the default values, if none were entered) by a common
