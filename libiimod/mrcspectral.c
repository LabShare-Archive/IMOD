...
/* sin is the slice input, low and high are between (0 - 1.0) */
int mrc_bandpass_filter(struct MRCslice *sin, double low, double high)
{
     ...
     maxsize = (sin->xsize > sin->ysize) ? sin->xsize / 2 : sin->ysize / 2;
     ...
     for (j = 0; j < sin->ysize; j++)
	  for(i = 0; i < sin->xsize; i++){
	       ...
	       if ((low > 0.0) && (dist < low))
		    /* mval = 0 */
		    mval = 1 / (1 + ((dist / low) * (dist / low) ));
	       else 
		    mval = 1.0;
	       
	       if ((high > 0.0) && (dist > high))
		    mval *= 1 / (1 + (((dist/high) * (dist/high)))); 
