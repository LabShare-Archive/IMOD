...
/* 
 * Incorporated into IMOD, with modifications to use Clarkson hull routines
 * $Id$ 
 */
...
#ifdef CLARKSON_HULL
static void tio_destroy(struct hullio* tio)
#else
...
#ifdef CLARKSON_HULL
static void tio2delaunay(struct hullio* tio_out, delaunay* d)
#else
