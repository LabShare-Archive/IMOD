...
short = output
type = FN
usage = Output file for reconstruction
tooltip = 
manpage = Output file for reconstruction.  If this option is not
...
tooltip = 
manpage = With this line, the program will assume that the section has
...
You may have any number of lines with exclude lists, but they cannot be 
...
tooltip = 
manpage = Use this line to specify the full size in X and Y of the
...
tooltip = 
manpage = If the input images have been binned, this line can be entered to
...
separated by spaces or commas.  Use more than one INCLUDE line if the numbers
...
tooltip = 
manpage = This line allows one to generate a reconstruction using the
...
tooltip = 
manpage = This line allows a mask to be applied so as to exclude from the
...
tooltip = 
manpage = This line allows one to specify the data mode of the output file,
...
usage = 
tooltip = 
manpage = This line allows an offset of DELANG degrees to be applied to all
...
tooltip = 
manpage = This line controls low-pass filtering with the radial weighting
...
tooltip = 
manpage = With this line, the input views will be replicated the number of
...
tooltip = 
manpage = With this line, the program will output one or more reprojections 
of the reconstructed slices at the given angles.  These reprojections should
match what Xyzproj (1) would produce, but will not match the original
projections if there were Z factors, local alignments, or any tilting about
the X-axis.
...
adding the first value then multiplying by the second one.
...
tooltip = 
manpage = This line allows one to shift the reconstructed slice in X or Z
...
tooltip = 
manpage = This line is used to allow multiple runs of Tilt to compute a subset
...
the program depends on the SLICE entry.  The program should be run
initially with SLICE -1 -1, which will cause it to create the output file
and write its header.  On successive runs with SLICE indicating a real range
of slices, the program will open the existing file, write only those slices, 
and not write the header when it is done.
...
local alignments are being used, use this line to enter the X and Y index
coordinates (numbered from 0) of the lower left corner of the subset within the
original images.  A FULLIMAGE line must also be entered.  If the aligned stack
is larger than the original images, use negative values.
...
tooltip = 
manpage = Use this line to specify a file containing a list of all tilt
...
tooltip = 
manpage = This line allows one to rotate the reconstruction around the X axis,
...
tooltip = 
manpage = Use this line to specify a file containing a list of tilts around
the X axis for the individual views.  A global tilt specified by the
XAXISTILT line, if any, will be added to these tilts.  If this file contains
...
tooltip = 
manpage = This line controls the order for interpolating an output slice
...
tooltip = 
manpage = Use this line to specify a file containing factors for adjusting the
