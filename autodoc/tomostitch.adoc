...
short = verbose
type = B
usage =  Output command lines being issued and other debugging output
tooltip =
manpage =
...
for each volume.  The default is to use the central half of the width of the
overlap zone; this may be too large if the overlap zones are very large and
contain much area without material.  Enter a width of 0,0 to use the original
...
density matching.  The default is to use the central half of the length of an
