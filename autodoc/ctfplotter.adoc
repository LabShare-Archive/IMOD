...
short = cache
type = I
usage = cache size in Megs
tooltip =
manpage = To speed up computation, ctfplotter uses a cache to hold slices it recently accessed. 
This option controls the cache size. If the user does not specify the option, it is
defaulted to 1000Megs.
