...
[Field = sampleSphere]
comment = If sampleSphere is set to 'full' or 'half' then in the first iteration
^dTheta and dPsi will be ignored, and dPhi and sampleInterval will be used.  If
^sampleSphere does not exist or is set to 'none' then the range specified by
^dPhi, dTheta, and dPsi will be searched.

[Field = sampleInterval]
comment = The interval in degrees at which the spherical surface will be
^sampled.
