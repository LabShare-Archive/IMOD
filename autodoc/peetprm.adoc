...
enum.0.tooltip = Generate a single-particle or use a user-specified reference.
enum.1.tooltip =  Generate a multi-particle reference.

[Field = reference]
KeyValueDelimiter = ==
comment == If flgFairReference = 1, a pair of integers [m, n] specifying
^m groups, each with n particles, be used to geneate a multi-particle
...
comment = f non-zero, align particles to have their Y axes
...
enum.0.tooltip = maximize absolute value of cross-correlation
enum.1.tooltip = maximize cross-correlation
