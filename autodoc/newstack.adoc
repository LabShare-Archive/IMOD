...
short = format
type = CH
usage = Type of output file: MRC, TIF or TIFF, HDF (case insensitive)
tooltip =
manpage = Set output file format to MRC, TIFF, or HDF (if the package supports
HDF) by entering MRC, TIF, TIFF, HDF, or any lower or mixed case variation on
...
short = chunk
type = IT
usage = Set chunk sizes in X, Y, and Z for 3D volumes stored in HDF files
tooltip =
manpage = Set chunk sizes in X, Y, and Z for 3D volumes stored in HDF files.
If this option is entered, the output file will be an HDF file with data
stored as a 3D volume, making it unnecessary to enter -3d or -format.  The
default is for 3D volumes to be stored with no tiling in X and Y and a chunk
size of 1 in Z, or with the tiling and chunk size of the first input file if
it an HDF file stored as a 3D volume.
