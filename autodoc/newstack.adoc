...
short = taper
type = IP
usage = Distance to taper at fill border and 0/1 for tapering outside/inside
tooltip = 
manpage = Distance in pixels over which to taper an image at a border between
real image and a filled area, and 0 for tapering outside or 1 for tapering
inside the border.  This performs the same operation as mrctaper(1).  The
