		...
		struct MRCheader *hout, struct Grap_options *opt)
{
     ...
     for(k = 0; k < hin1->nz; k++){
	  mrc_read_slice((void *)sin1->data.b, hin1->fp, hin1, k, 'z');
	  if (!autoc)
	       mrc_read_slice((void *)sin2->data.b, hin2->fp, hin2, k, 'z');
	  corr_conj(sin1->data.f, sin2->data.f, size);
	  mrc_write_slice((void *)sin1->data.b, hout->fp, hout, k, 'z');
     }
...
/*     printf(" %d %d %d %d\n", llx, lly, urx, ury); */
     ...
     if (opt->add2file == IP_APPEND_ADD){
	  ...
	  hout->nz++;
	  mrc_write_slice((void *)cslice->data.f, hout->fp, 
			  hout, hout->nz - 1, 'z');
	  ...
	  mrc_head_write(hout->fp, hout);
	  mrc_write_slice((void *)cslice->data.f, hout->fp, hout, 0, 'z');
     }
