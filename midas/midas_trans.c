/*  IMOD VERSION 2.50
 *
 *  midas_trans.c -- Transformation functions used in midas.
 *
...
int new_view(struct Midas_view *vw)
{
     ...
     for (i = 0; i < 3; i++) {
	  vw->incindex[i] = 4;
	  vw->increment[i] = getIncrement(4, i);
     }
     for (i = 0; i < 5; i++)
	  vw->paramstate[i] = -999.;
     return(0);
...
{
     int i, k, ix, ixy, iy, ind, ipclo, ipchi, ned;
     FILE *file;
     ...
     if (vw->plname) {
	  ...
	  set_mont_pieces(vw);
	  vw->incindex[2]--;
          vw->increment[2] = getIncrement(vw->incindex[2], 2);
		      ...
		      float *curerrx, float *curerry, int localonly)
{
     ...
     int nsum, neighpc, neighvar, edge, m, ixy;
